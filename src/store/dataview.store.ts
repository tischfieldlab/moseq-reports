...
import { DatasetsState, LabelMapRecord } from './datasets.types';
...
const DataviewModule: Module<DataviewState, RootState> = {
    ...
    getters: {
        selectedSyllableAs: (state, _, rootState) => (countMethod: CountMethod) => {
            const lm = (rootState as unknown as DatasetsState).label_map;
            const from = state.countMethod.toLowerCase();
            const to = countMethod.toLowerCase();
            const result = lm.find((row: LabelMapRecord) => row[from] === state.selectedSyllable);
            return result ? result[to] : -5;
        },
        ...
        selectedSyllables: (state, getters) => {
            return state.moduleIdFilter.length === 0 ? getters.availableModuleIds : state.moduleIdFilter;
        },
        ...
        availableGroups: (state, getters, rootState) => {
            return (rootState as unknown as DatasetsState).groups;
        },
    },
    mutations: {
        setName(state: DataviewState, name: string) {
            state.name = name;
        },
        setColor(state: DataviewState, color: string) {
            state.color = color;
        },
        setLoading(state: DataviewState, loading: boolean) {
            state.loading = loading;
        },
        setGroupColors(state: DataviewState, groupColors: string[]) {
            state.groupColors = groupColors;
        },
        setView(state: DataviewState, payload: DataviewPayload) {
        ...
        setSelectedSyllable(state: DataviewState, selectedSyllable: number) {
            state.selectedSyllable = selectedSyllable;
        },
        publishDataset(state: DataviewState, payload: PublishDatasetPayload) {
            state.views = { ...state.views, [`${payload.owner}/${payload.name}`]: payload };
        },
        unpublishDataset(state: DataviewState, payload: UnpublishDatasetPayload) {
            const { [`${payload.owner}/${payload.name}`]: _, ...rest }: {[index: string]:any} = state.views;
            state.views = rest;
        },
    },
    actions: {
        ...
        switchCountMethod(context, payload: CountMethod) {
            ...
            const lm = (context.rootState as unknown as DatasetsState).label_map;
            const from = context.state.countMethod.toLowerCase();
            const filterSyllables = context.state.moduleIdFilter.map((id) => {
                const result = lm.find((row: LabelMapRecord) => row[from] === id);
                return result ? result[payload.toLocaleLowerCase()] : undefined;
            });
...
function bindStore(name: string) {
