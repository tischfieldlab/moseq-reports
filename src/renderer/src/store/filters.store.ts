    ...
    actions: {
        async serializeFilters(context): Promise<any> {
            const dehydrated = context.state.items.map(async (id) => {
                return [
                    id.split("/")[1],
                    await context.dispatch(`${id}/serialize`, undefined, {
                        root: true,
                    }),
        ...
        async loadFilters(context, filters: any) {
            ...
            const namespace = getModuleNamespace(
                store,
                context.state
            ) as string;
            Object.entries(filters).forEach(async ([name, filter]) => {
                ...
                if (existing.includes(fullpath)) {
                    await store.dispatch(`${fullpath}/load`, filter, {
                        root: true,
                    });
                } else {
        ...
        removeFilter(context, namespace: string) {
            ...
            const winsUsingFilter =
                context.rootGetters["datawindows/windowsUsingDataView"](
                    namespace
                );
