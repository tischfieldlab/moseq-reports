...
import {
    ...
    isValidHeight,
    isValidWidth,
} from "../components/Core/Window/util";
...
const DataWindowModule: Module<DataWindowState, RootState> = {
    ...
    mutations: {
        ...
        updateComponentLayout(state, payload: UpdateComponentLayoutPayload) {
            ...
            const deltaY: number = payload.height
                ? payload.height
                : state.height;
            ...
            if (payload.position_y !== undefined) {
                ...
                if (payload.position_y + deltaY > maxY)
                    payload.position_y = maxY - state.height - 65;
                if (state.height === 0 && payload.position_y > maxY - 65)
                    payload.position_y = maxY - 65;
        ...
        updateComponentSettings(
            state,
            payload: UpdateComponentSettingsPayload
        ) {
            stateMerge(state.settings, payload.settings);
        },
