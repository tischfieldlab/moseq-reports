...
const DataviewModule: Module<DataviewState, RootState> = {
    ...
    getters: {
        selectedSyllableAs:
            (state, _, rootState) => (countMethod: CountMethod) => {
                const lm = ((rootState as any).datasets as DatasetsState)
                    .label_map;
                const from = state.countMethod.toLowerCase();
                const to = countMethod.toLowerCase();
                const result = lm.find(
                    (row) => row[from] === state.selectedSyllable
                );
                if (result !== undefined) {
                    return result[to];
                } else {
    ...
    actions: {
        ...
        switchCountMethod(context, payload: CountMethod) {
            ...
            const newSelectedSyllable =
                context.getters.selectedSyllableAs(payload);
        ...
        async updateModuleIdFilters(context, payload: number[]) {
            ...
            if (
                ...
                !context.state.moduleIdFilter.includes(
                    context.state.selectedSyllable
                )
            ) {
                context.commit(
                    "setSelectedSyllable",
                    context.state.moduleIdFilter[0]
                );
            }
        },
        ...
        async updateView(context, payload: DataviewPayload) {
            ...
            try {
                ...
                payload.countMethod =
                    payload.countMethod || context.state.countMethod;
