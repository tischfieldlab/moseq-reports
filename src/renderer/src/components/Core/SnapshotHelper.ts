...
export async function SnapshotWorkspace() {
    ...
    if (toSnapshot.length <= 0) {
        showSaveErrorToast(
            "There are not any items to snapshot!",
            "workspace snapshot"
        );
        return;
    }
...
export function composite_images(
    images: SubImage[],
...
export function resolveTarget(target: Vue): {
    ...
    const explicit = (
        target.$el.hasAttribute(eattr)
            ? target.$el
            : target.$el.querySelector(`[${eattr}]`)
    ) as HTMLElement;
    if (explicit !== null) {
