    ...
    return (mapData as any[][]).map((row) => {
        return Object.fromEntries(
            coldefs.map((cd) => {
                return [
                    cd.dest,
                    Array.isArray(row) ? row[cd.idx] : row[cd.src],
                ];
            })
        );
    });
...
function Compare(
    ...
    if (direction === SortDirection.Asc) {
        ...
        throw new Error(
            `Unsupported direction '${direction}' in sort for column '${column}'`
        );
    }
}
...
export function filterBy(data: object[], op: FilterOperation) {
    return data.filter((row) =>
        Object.entries(op.filters).every(([col, criterum]) =>
            criterum.includes(row[col])
        )
    );
}
...
export function aggregate(data: object[], op: AggregateOperation) {
    const grouper = (item) =>
        (op.groupby as string[]).map((c) => item[c]).toString();
            ...
            ...Object.entries(op.aggregate).flatMap(([col, stats]) => {
                if (Array.isArray(stats)) {
                    return stats.map((stat) => {
                        return [
                            `${col}_${stat}`,
                            (statops[stat] as any)(vals.map((v) => v[col])),
                        ];
                    });
                } else {
                    return [
                        [col, (statops[stats] as any)(vals.map((v) => v[col]))],
                    ];
                }
            }),
        ]);
            ...
            zip.on("ready", async () => {
                try {
                    ...
                    if (entry !== null) {
                        ...
                        throw new Error(
                            `Entry ${entryname} is missing from data file!`
                        );
                    }
                } catch (e) {
                    reject(`${e}: ${entryname}`);
