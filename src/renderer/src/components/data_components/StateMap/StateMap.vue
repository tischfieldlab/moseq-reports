...
import {
    ...
    SubImage,
    targetToDataURI,
} from "../../../components/Core/SnapshotHelper";
...
export default mixins(WindowMixin, LoadingMixin).extend({
    ...
    created() {
        if (
            this.settings.plot_group === undefined ||
            this.settings.plot_group === ""
        ) {
        ...
        if (
            this.settings.relative_diff_group === undefined ||
            this.settings.relative_diff_group === ""
        ) {
            this.$store.commit(`${this.id}/updateComponentSettings`, {
                id: this.id,
                settings: {
    ...
    computed: {
            ...
            switch (this.settings.layout) {
                case "grid":
                    return {
                        ...
                        avoidOverlapPadding:
                            this.settings.grid_settings.avoid_overlap_padding,
                    };
                case "circle":
                    return {
                        name: "circle",
                        avoidOverlap:
                            this.settings.circle_settings.avoid_overlap,
                ...
                case "concentric":
                    return {
                        name: "concentric",
                        avoidOverlap:
                            this.settings.concentric_settings.avoid_overlap,
                        clockwise: this.settings.concentric_settings.clockwise,
                        equidistant:
                            this.settings.concentric_settings.equidistant,
                        minNodeSpacing:
                            this.settings.concentric_settings.min_node_spacing,
                    };
                case "avsdf":
                    return {
                        name: "avsdf",
                        nodeSeparation:
                            this.settings.avsdf_settings.node_separation,
                    };
                case "fcose":
                    return {
                        name: "fcose",
                        nodeSeparation:
                            this.settings.fcose_settings.node_separation,
                        nodeRepulsion:
                            this.settings.fcose_settings.node_repulsion,
                        idealEdgeLength:
                            this.settings.fcose_settings.ideal_edge_length,
                    };
                case "cise":
                    return {
                        name: "cise",
                        nodeSeparation:
                            this.settings.cise_settings.node_separation,
                        nodeRepulsion:
                            this.settings.cise_settings.node_repulsion,
                        idealEdgeLength:
                            this.settings.cise_settings.ideal_edge_length,
                    };
            }
            ...
            return [
                ...
                },
                {
                    ...
                    style: {
                        ...
                        opacity: this.settings.use_opacity
                            ? (d) => this.scale.o(d.data("weight"))
                            : 1,
                        "line-color": (d) => this.scale.z(d.data("weight")),
                        "target-arrow-color": (d) =>
                            this.scale.z(d.data("weight")),
        ...
        elements(): any[] {
            const trans = this.raw_data.transitions.filter(
                (row) => row.group === this.settings.plot_group
            );
            const transSum = trans.reduce((acc, curr) => acc + curr.raw, 0);
            const relTrans = this.raw_data.transitions.filter(
                (row) => row.group === this.settings.relative_diff_group
            );
            const relTransSum = relTrans.reduce(
            ...
            if (
                showRelDiff &&
                (relTrans === undefined || relTrans.length === 0)
            ) {
                return [];
            }
            ...
            for (const s of this.activeSyllables) {
                const n = this.raw_data.usages.find(
                    (row) => row.syllable === s
                );
                if (n !== undefined) {
                    elements.push({
            ...
            for (const [i, t] of trans.entries()) {
                if (
                    this.activeSyllables.includes(t.row_id) &&
                    this.activeSyllables.includes(t.col_id)
                ) {
                    const val = t.raw / transSum;
                    if (
                        showRelDiff &&
                        (t.row_id !== relTrans[i].row_id ||
                            t.col_id !== relTrans[i].col_id)
                    ) {
                        /* tslint:disable-next-line:no-console */
                        console.warn(
                            "primary and relative mismatch",
                            t,
                            relTrans[i]
                        );
                    }
        ...
        scale(): any {
            const r = scaleLinear()
                .domain(
                    extent(this.raw_data.usages.map((n) => n.usage)) as [
                        number,
                        number
                    ]
                )
            ...
            const abstransMax =
                links.length > 0
                    ? (max(links, (d) => Math.abs(d.data.weight)) as number)
                    : 1;
            const transExtent =
                links.length > 0
                    ? (extent(links, (d) => d.data.weight) as [number, number])
                    : 1;
            ...
            if (transExtent[0] < 0) {
                ...
                t = scaleLinear().domain([0, abstransMax]).range([1, 10]);
                ...
                zo = scaleDiverging(
                    (this.settings.use_opacity
                        ? GetScaleWithOpacity(this.settings.colorscale, zo)
                        : GetScale(this.settings.colorscale)) as (
                        t: number
                    ) => string
                ).domain(transDomain as [number, number, number]);
            } else {
                transDomain = [0, abstransMax] as [number, number];
                t = scaleLinear().domain(transDomain).range([1, 10]);

                o = scaleLinear().domain(transDomain).range([0, 1]);

                z = scaleSequential(
                    GetScale(this.settings.colorscale) as (t: number) => string
                ).domain(transDomain as [number, number]);
                zo = scaleSequential(
                    GetScaleWithOpacity(this.settings.colorscale, o)
                ).domain(transDomain as [number, number]);
            }
            return { r, t, o, z, zo };
        },
        sourceData(): any {
            ...
            const transSource =
                this.$store.getters[`datasets/resolve`]("transitions");
            ...
            const filterGroups = [
                ...(showRelDiff
                    ? [this.settings.plot_group, relDiffGroup]
                    : [this.settings.plot_group]),
            ];
            ...
            const usageFilters = [
                {
                    ...
                    columns: [
                        [
                            `usage_${this.dataview.countMethod.toLowerCase()}`,
                            "usage",
                        ],
                        ["group", "group"],
                        [
                            `id_${this.dataview.countMethod.toLowerCase()}`,
                            "syllable",
                        ],
                    ],
                },
            ...
            const transFilters = [
                {
                    ...
                    columns: [
                        ["default_group", "group"],
                        [
                            `row_id_${this.dataview.countMethod.toLowerCase()}`,
                            "row_id",
                        ],
                        [
                            `col_id_${this.dataview.countMethod.toLowerCase()}`,
                            "col_id",
                        ],
                        "raw",
                    ],
            ...
            return {
                ...
                is_valid:
                    filterGroups.filter((g) => g !== "" && g !== undefined)
                        .length > 0,
            };
        },
        activeSyllables(): number[] {
            if (this.dataview.moduleIdFilter.length === 0) {
                return this.$store.getters[
                    `${this.datasource}/availableModuleIds`
                ];
            } else {
    ...
    methods: {
        ...
        onNodeClick(event) {
            if (event.target && event.target._private.data.id) {
                this.selectedSyllable = Number.parseInt(
        ...
        snapshot(options: SnapshotOptions): Promise<string> {
            return new Promise((resolve, reject) => {
                ...
                .then(async (graphSubImage) => {
                    const lel = (this.$refs.legendHost as Vue)
                        .$el as HTMLElement;
                    const legend = {
                        dataURI: await targetToDataURI(
                            this.$refs.legendHost as Vue,
                            options
                        ),
                        pos_x: 0,
