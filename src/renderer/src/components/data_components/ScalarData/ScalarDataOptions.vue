    ...
    <b-container fluid>
        ...
        <b-row>
            <b-col>
                ...
                <div v-show="show_boxplot" class="figure-caption">
...
export default mixins(WindowMixin).extend({
    computed: {
        ...
        boxplot_whisker_description: {
            get(): string {
                return this.whisker_options.find(
                    (wo) => wo.value === this.boxplot_whiskers
                )!.description;
            },
        },
        show_violinplot: {
    ...
    data() {
        return {
            whisker_options: [
                {
                    ...
                    text: "Tukey",
                    description:
                        "Whiskers extend up to 1.5 * IQR from 25th and 75th percentile",
                },
                {
            ...
            metric_options: Object.entries(availableMetrics).map(
                ([metric, info]) => ({ value: metric, text: info.title })
            ),
        };
    },
});
