<template>
    <svg ref="canvas" :width="outsideWidth" :height="outsideHeight">
        <template v-for="group_name in selectedGroups">
            <g
                v-if="selectedGroups.includes(group_name)"
                :key="group_name"
                :class="{ heatmap: true, [group_name]: true }"
                :transform="`translate(${scale.o(group_name)},0)`"
            >
                <text class="label" :x="scale.o.bandwidth() / 2" :y="10">
                    {{ group_name }}
                </text>
                <g class="heat-tiles" transform="translate(0, 20)">
                    <template
                        v-for="(rows, row_id) in group_transitions(group_name)"
                    >
        ...
        <ColorScaleLegend
            ...
            :width="width / 2"
            :height="50"
            :transform="`translate(${outsideWidth / 2}, ${
                scale.o.bandwidth() + 40
            })`"
        />
...
export default mixins(LoadingMixin, WindowMixin).extend({
    ...
    computed: {
        ...
        scale(): any {
            ...
            const y = scaleBand()
                ...
                .range([o.bandwidth(), 0])
                .padding(0);
            const absz = scaleSequential(this.abs_colormap).domain([
                this.abs_vmin,
                this.abs_vmax,
            ]);
            const relz = scaleSequential(this.rel_colormap).domain([
                this.rel_vmin,
                this.rel_vmax,
            ]);
            const c = scaleLinear()
        ...
        dataspec() {
            ...
            if (this.dataview.countMethod === CountMethod.Usage) {
                ds =
                    this.$store.getters[`datasets/resolve`](
                        "transitions_usage"
                    );
                ...
                throw new Error(
                    `Count method ${this.dataview.countMethod} is not supported`
                );
            }
            let syllables;
    ...
    watch: {
        dataspec: {
            handler(newValue) {
                LoadData(newValue[0], newValue[1], true).then((data) =>
    ...
    methods: {
            ...
            for (const g of this.selectedGroups) {
                for (let i = 0; i < absTrans[g].length; i++) {
                    ...
                    for (let j = 0; j < relTrans[g][i].length; j++) {
                        relTrans[g][i][j] =
                            relTrans[g][i][j] - relTrans[root][i][j];
                    }
        ...
        highlight_transition(event: MouseEvent) {
            if (
                event &&
                event.srcElement &&
                event.srcElement instanceof SVGRectElement
            ) {
