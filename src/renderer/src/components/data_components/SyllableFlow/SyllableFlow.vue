...
import {
    ...
    NodeAlignment,
    ColoringMode,
} from "../../../components/Charts/Sankey/Sankey.types";
...
function default_tooltip_formatter(hoverItem, that) {
    if (hoverItem !== undefined) {
        ...
        if (hi.type === "node") {
            ...
            return `Transition ${
                hi.id
            }<br />P(t) = ${hi.real_value.toExponential(3)}`;
        }
    }
    return "";
}
...
export default mixins(WindowMixin, LoadingMixin).extend({
    ...
    computed: {
        ...
        sourceData(): any {
            ...
            const filterGroups = [
                ...(this.settings.show_relative_diff
        ...
        graph(): { nodes: Node[]; links: Link[] } {
            ...
            // incoming links
            for (const [i, t] of trans
                .filter((row) => row.col_id === this.selectedSyllable)
                .entries()) {
                if (
                    this.activeSyllables.includes(t.row_id) &&
                    t.row_id !== this.selectedSyllable
                ) {
            ...
            // outgoing links
            for (const [i, t] of trans
                .filter((row) => row.row_id === this.selectedSyllable)
                .entries()) {
                if (
                    this.activeSyllables.includes(t.col_id) &&
                    t.col_id !== this.selectedSyllable
                ) {
                    const val = t.raw / transSum;
                    if (val > this.settings.prune_threshold) {
