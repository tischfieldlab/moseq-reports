...
import {
    LinePlotSVG,
    LinePlotCanvas,
} from "../../../components/Charts/LinePlot";
    ...
    computed: {
        ...
        syllable_ordering(): any[] {
            if (this.settings.syllable_order_type === OrderingType.Natural) {
                return this.$store.getters[
                    `${this.datasource}/selectedSyllables`
                ];
            } else if (
                this.settings.syllable_order_type === OrderingType.Dataset
            ) {
                if (
                    this.dataview.views[
                        this.settings.syllable_order_dataset
                    ] !== undefined
                ) {
                    return this.dataview.views[
                        this.settings.syllable_order_dataset
                    ].data.map((d) => Number.parseInt(d, 10));
                }
            } else if (
                this.settings.syllable_order_type === OrderingType.Value
            ) {
                return this.aggregateView
                    .filter(
                        (u) =>
                            u.group ===
                            this.settings.syllable_order_group_value.toString()
                    )
                    .sort((a, b) => {
                        if (
                            this.settings.syllable_order_direction ===
                            SortOrderDirection.Dec
                        ) {
                ...
                this.settings.syllable_order_type === OrderingType.Computed
            ) {
                const syllables =
                    this.$store.getters[`${this.datasource}/selectedSyllables`];
                const minData = this.aggregateView.filter(
                    (u) =>
                        u.group ===
                        this.settings.syllable_order_diff_minuend.toString()
                );
                const subData = this.aggregateView.filter(
                    (u) =>
                        u.group ===
                        this.settings.syllable_order_diff_subtrahend.toString()
                );

                return syllables
                    .map((s) => {
                        const min =
                            minData.find((p) => p.syllable === s)?.value || 0;
                        const sub =
                            subData.find((p) => p.syllable === s)?.value || 0;
                        return {
                            syllable: s,
                            value: min - sub,
                        };
                ...
                // tslint:disable-next-line:no-console
                console.warn(
                    `Unsupported group order type ${this.settings.syllable_order_type}`
                );
            }
            return [];
        },
        ...
        groupNames(): string[] {
            if (this.settings.group_order_type === OrderingType.Natural) {
                ...
                this.settings.group_order_type === OrderingType.Dataset
            ) {
                if (
                    this.dataview.views[this.settings.group_order_dataset] !==
                    undefined
                ) {
                    return this.dataview.views[
                        this.settings.group_order_dataset
                    ].data;
                }
            } else {
                // tslint:disable-next-line:no-console
                console.warn(
                    `Unsupported group order type ${this.settings.group_order_type}`
                );
            }
            return [];
        },
        groupColors(): string[] {
            return this.groupNames.map(
                (gn) =>
                    this.dataview.groupColors[
                        this.dataview.selectedGroups.indexOf(gn)
                    ]
            );
        },
