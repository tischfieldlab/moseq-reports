<template>
    <div>
        <b-dropdown
            text="Add Filter"
            class="float-right add-filter-button"
            size="sm"
        >
        ...
        <template v-for="(value, key) in localFilters">
            <b-input-group size="sm" :key="key">
                <b-input-group-prepend
                    is-text
                    :title="`${inferDataTypeForColumn(key)} datatype`"
                    v-b-tooltip.hover
                >
                    {{ key }}
                </b-input-group-prepend>
        ...
        <div class="special-token-container">
            ...
            <template v-for="(value, key) in SpecialTokens">
                <b-tag
                    ...
                    variant="info"
                >
                    <span :title="JSON.stringify(value)" v-b-tooltip.hover>{{
                        key
                    }}</span>
                </b-tag>
            </template>
...
export default Vue.extend({
    ...
    methods: {
        ...
        columnAlreadyInFilter(colName) {
            return Object.getOwnPropertyNames(this.localFilters).includes(
                colName
            );
        },
