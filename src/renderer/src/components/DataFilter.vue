    ...
    <b-card class="shadow datafilter">
        <template v-slot:header>
            <div :style="{ background: color, color: getContrast(color) }">
                ...
                <b-popover
                    :target="$id(datasource)"
                    triggers="click blur"
                    placement="top"
                >
                    <template v-slot:title
                        >Dataview `{{ filter_name }}` Color</template
                    >
        ...
        <b-collapse :visible="is_expanded" :id="$id('filter-collapse')">
            <b-overlay :show="is_loading" no-fade>
                <div class="container">
                    ...
                    <b-input-group
                        prepend="Count Method"
                        class="filter-item count-method"
                    >
                        <b-form-select
                            v-model="selectedCountMethod"
                            :options="countMethods"
                        />
                    </b-input-group>

                    <b-input-group
                        prepend="Selected Syllable"
                        class="filter-item selected-syllable"
                    >
                        <b-button
                            class="prev"
                            size="sm"
...
export default Vue.component("datafilter", {
        ...
        selectedCountMethod: {
            ...
            set(value: CountMethod) {
                if (value !== this.selectedCountMethod) {
                    this.$store.dispatch(
                        `${this.datasource}/switchCountMethod`,
                        value
                    );
                }
            },
    ...
    methods: {
        ...
        isSyllableAvailable(syllableId: number) {
            return (
                this.syllableIdOptions.find((so) => so.value === syllableId) !==
                undefined
            );
        },
    },
