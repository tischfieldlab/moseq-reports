...
export default mixins(LoadingMixin).extend({
    ...
    computed: {
        ...
        groupedData(): { [series: string]: {}[] } {
            const series = [
                ...new Set(this.data.map((d) => d[this.seriesKey])),
            ];
            const kludges = this.varOrdering.filter(
                ...
                series.map((seriesValue) => {
                    return [
                        ...
                        this.data
                            ...
                            .sort(
                                (a, b) =>
                                    this.varOrdering.indexOf(a[this.varKey]) -
                                    this.varOrdering.indexOf(b[this.varKey])
                            ),
                    ];
                })
            );
    ...
    methods: {
        isPointValid(d: object) {
            return (
                !Number.isNaN(this.scale.x(d[this.varKey])) &&
