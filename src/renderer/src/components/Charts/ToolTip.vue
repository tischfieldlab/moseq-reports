<template>
    <Portal>
        <div
            :id="$id('tooltip-contents')"
            class="tooltip_contents"
            v-show="show"
        >
...
export default Vue.extend({
    ...
    data() {
        return {
            ...
            virtualElement: {
                getBoundingClientRect: undefined as (() => DOMRect) | undefined,
            } as VirtualElement,
        };
    },
    mounted() {
        this.$nextTick().then(() => {
            this.virtualElement.getBoundingClientRect =
                this.generateGetBoundingClientRect(this.position);
            this.instance = createPopper(
                this.virtualElement as VirtualElement,
                document.querySelector(
                    this.$idRef("tooltip-contents")
                ) as HTMLElement,
                {
                    placement: "right",
                    modifiers: [
    ...
    methods: {
        ...
        generateGetBoundingClientRect(position: {
            x: number;
            y: number;
        }): () => DOMRect {
