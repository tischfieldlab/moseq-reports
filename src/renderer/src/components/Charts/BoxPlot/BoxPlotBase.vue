...
export default mixins(LoadingMixin).extend({
    props: {
        ...
        tooltipFormatter: {
            type: Function as PropType<
                (item: DataPoint | GroupStats, sender: Vue) => string
            >,
    ...
    computed: {
        ...
        innerWidth(): number {
            ...
            this.rotate_labels =
                this.label_stats.longest > width / this.label_stats.count;
        ...
        scale(): any {
            ...
            const orderedLabels = this.groupedData
                ...
                .sort(
                    (a, b) =>
                        this.groupLabels.indexOf(a) -
                        this.groupLabels.indexOf(b)
                );
        ...
        fences() {
            switch (this.whisker_type) {
                ...
                case WhiskerType.TUKEY:
                    return {
                        lower: (gs: GroupStats) =>
                            Math.max(gs.q1 - 1.5 * gs.iqr, gs.min),
                        upper: (gs: GroupStats) =>
                            Math.min(gs.q3 + 1.5 * gs.iqr, gs.max),
                    };
                default:
                    throw new Error(
                        `Unsupported Whisker Type ${this.whisker_type}!`
                    );
            }
        },
        violinArea(): any {
        ...
        actuallyShowPoints(): boolean {
            ...
            if (tooMany) {
                ...
                console.warn(
                    `Too many points (${this.points.length}): disableing show points`
                );
            }
    ...
    methods: {
        ...
        is_outlier(node: DataPoint): boolean {
            ...
            if (group) {
                return (
                    node.value < this.fences.lower(group) ||
