...
const exposedMethods = {
    prepareData(
        ...
        const groupedData = Object.entries(
            groupby(points, (p) => p.group, groupLabels)
        )
            .sort((a, b) => a[0].localeCompare(b[0]))
            .map(([key, values]) => {
                return exposedMethods.computeGroupStats(
                    values.map((dp) => dp.value),
                    key,
                    y,
                    kdeScale
                );
        ...
        const kdeMax = Math.max(
            ...groupedData.map((g) => Math.max(...g.kde.map((k) => k[1])))
        );
        ...
        groupLabels.map((g) => {
            ...
            for (const b of indv) {
                ...
                if (intersects(b.jitter, b.value)) {
                    ...
                    do {
                        const dy = Math.sqrt(
                            radius2 - (scaleY(a!.value) - scaleY(b.value)) ** 2
                        );
                        const j = a!.jitter + dy;
...
function kernelDensityEstimator(
    ...
    return (sample: number[]) => {
        return x.map((y) => [
            y,
            mean(sample, (v: number) => kernel(y - v)),
        ]) as number[][];
    };
}
