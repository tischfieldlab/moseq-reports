...
export default mixins(BoxPlotBase, CanvasMixin).extend({
    ...
    methods: {
        draw() {
            (this as any).emitStartLoading();
            this.$forceNextTick().then(() => {
                const ctx = this.canvas.cxt;
                if (ctx === null) {
                    (this as any).emitFinishLoading();
                    return; // bail out
                }
                ctx.save();
                // clear canvas
        ...
        drawBoxPlotNode(ctx: CanvasRenderingContext2D, node: GroupStats) {
            ...
            ctx.beginPath();
            ctx.moveTo(
                this.scale.x(node.group) + this.halfBandwith,
            ...
            ctx.beginPath();
            ctx.moveTo(
                this.scale.x(node.group) + this.quaterBandwith,
                this.scale.y(this.fences.lower(node))
            );
            ctx.lineTo(
                this.scale.x(node.group) + this.quaterBandwith * 3,
                this.scale.y(this.fences.lower(node))
            );
            ctx.stroke();
            ...
            ctx.lineTo(
                this.scale.x(node.group) + this.halfBandwith * 2,
        ...
        drawAxisX(ctx: CanvasRenderingContext2D) {
            ...
            ctx.fillText(
                this.xAxisTitle,
                this.innerWidth / 2,
                this.innerHeight + this.xAxisLabelYPos
            );
            ctx.restore();
        },
