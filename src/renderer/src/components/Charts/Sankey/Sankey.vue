        ...
        >
            <template v-if="graph.nodes.length > 0 && graph.links.length > 0">
                ...
                <g>
                    ...
                    <text
                        class="axis-label"
                        :transform="`translate(${width - margin.right / 2}, ${
                            innerHeight / 2 + margin.top
                        }) rotate(-90)`"
                    >
                        Outgoing
                    ...
                    >
                        <rect
                            ...
                            :fill="
                                color(scale.n(n[nodeColorProperty])).darker(0.5)
                            "
                            :data-nodeid="n.id"
                        ></rect>
                        <text
    ...
    computed: {
        ...
        scale(): any {
            ...
            if (this.nodeColorMode === ColoringMode.Categorical) {
                n = scaleOrdinal<string, string>().range(
                    GetScale(this.categoricalColormap) as string[]
                );
                if (this.nodeIdSuperset !== undefined) {
                    ...
                    n = n.domain(
                        this.data.nodes.map(
                            (node) => node[this.nodeColorProperty]
                        )
                    );
                }
            }
            ...
            if (this.linkColorMode === ColoringMode.Quantitative) {
                const abstransMax =
                    max(this.data.links, (d) =>
                        Math.abs(d[this.linkColorProperty])
                    ) || 1;
                li = scaleDiverging(
                    GetScale(this.quantitativeColormap) as (t: number) => string
                ).domain([-abstransMax, 0, abstransMax]);
                l = (link: Link) =>
                    color(li(link[this.linkColorProperty]) as string);
            } else {
                li = scaleOrdinal()
                    .range(GetScale(this.categoricalColormap) as string[])
                    .domain(
                        this.data.links.map((d) => d[this.linkColorProperty])
                    );
                l = (link: Link) =>
                    color(li(link[this.linkColorProperty]) as string)!.brighter(
                        0.5
                    );
            }
            return { n, l, li };
        ...
        handleHover(event: MouseEvent) {
            if (event && event.target !== null) {
                ...
                if (target.dataset.nodeid) {
                    ...
                    this.hoverItem = this.data.nodes.find(
                        (n) => n.id.toString() === target.dataset.nodeid
                    );
                    return;
                }
                ...
                if (target.dataset.transitionid) {
                    ...
                    this.hoverItem = this.data.links.find(
                        (n) => n.id.toString() === target.dataset.transitionid
                    );
                    return;
                }
            }
