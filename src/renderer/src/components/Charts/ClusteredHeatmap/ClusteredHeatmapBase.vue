...
import {
    ...
    elbowH,
    elbowV,
} from "../../../components/Charts/D3Clustering";
...
export default Vue.extend({
    ...
    computed: {
        ...
        dims(): any {
            const rtreeWidth = this.isRowsHClustered
                ? Math.min(this.innerWidth * 0.1, 50)
                : 0;
            const ctreeHeight = this.isColumnsHClustered
                ? Math.min(this.innerHeight * 0.1, 50)
                : 0;
            const yaxisWidth = 45;
            let xaxisHeight = 45;
            ...
            this.rotate_labels =
                this.label_stats.longest > heatWidth / this.label_stats.count;
            if (this.rotate_labels) {
                const rotatedHeight =
                    Math.cos(45 * (Math.PI / 180)) * this.label_stats.longest;
                xaxisHeight = xaxisLabelYOffset = rotatedHeight + 45;
            }

            const heatHeight =
                this.innerHeight - ctreeHeight - xaxisHeight - legendHeight;
        ...
        isColumnsClustered(): boolean {
            return [OrderingType.HCluster, OrderingType.KCluster].includes(
                this.columnOrderType as OrderingType
            );
        },
        isRowsClustered(): boolean {
            return [OrderingType.HCluster, OrderingType.KCluster].includes(
                this.rowOrderType as OrderingType
            );
        },
            ...
            switch (this.columnOrderType) {
                ...
                case OrderingType.Value:
                    return this.data
                        .filter(
                            (u) =>
                                u[this.rowKey].toString() ===
                                this.columnOrderValue.toString()
                        )
                        .sort((a, b) => {
                            if (
            ...
            switch (this.rowOrderType) {
                ...
                case OrderingType.Value:
                    return this.data
                        .filter(
                            (u) =>
                                u[this.columnKey].toString() ===
                                this.rowOrderValue.toString()
                        )
                        .sort((a, b) => {
                            if (
                                this.rowOrderDirection ===
                                SortOrderDirection.Asc
                            ) {
                                return b[this.valueKey] - a[this.valueKey];
                            } else {
                ...
                default:
                    return [
                        ...new Set(this.data.map((u) => u[this.rowKey])),
                    ].sort((a, b) => a - b);
            }
        },
        colormap(): any {
        ...
        scale(): any {
            ...
            const y = scaleBand()
                ...
                .padding(0);
            const vals = (this.data || []).map(
                (n) => n[this.valueKey] as number
            );
            const ext = [
        ...
        columnLinks(): any[] {
            ...
            return cluster()
                .size([this.dims.ctree.w, this.dims.ctree.h])(
                    this.columnHierarchy as any
                )
                .links() as any;
        },
        rowLinks(): any[] {
            ...
            return cluster()
                .size([this.dims.rtree.h, this.dims.rtree.w])(
                    this.rowHierarchy as any
                )
                .links() as any;
        },
        tooltip_text(): string {
            if (this.hoverItem !== undefined) {
                return (this.tooltipFormatter as (itm, that) => string)(
                    this.hoverItem,
                    this
                );
            }
        ...
        async clusterColumns() {
            if (this.data !== null && this.data.length > 0) {
                if (this.columnOrderType === OrderingType.HCluster) {
                    const tree = await worker.clusterColumns(
                        this.data,
                        this.columnKey,
                        this.valueKey,
                        {
                            ...
                            distance: this
                                .columnClusterDistance as HClusterDistance,
                            linkage: this
                                .columnClusterLinkage as HClusterLinkage,
                        }
                    );
                    ...
                    const data = await worker.clusterColumns(
                        this.data,
                        this.columnKey,
                        this.valueKey,
                        {
                    ...
                    const cco = data
                        .reduce(
                            (acc, c, idx) =>
                                acc.concat(c.clusterInd, [-1 * (idx + 1)]),
                            []
                        )
                        .map(
                            (idx: number) =>
                                this.groupLabels[idx] || idx.toString()
                        );
                    cco.pop();
        ...
        async clusterRows() {
            if (this.data !== null && this.data.length > 0) {
                if (this.rowOrderType === OrderingType.HCluster) {
                    const tree = await worker.clusterRows(
                        this.data,
                        this.rowKey,
                        this.valueKey,
                        {
                            type: OrderingType.HCluster,
                            distance: this
                                .rowClusterDistance as HClusterDistance,
