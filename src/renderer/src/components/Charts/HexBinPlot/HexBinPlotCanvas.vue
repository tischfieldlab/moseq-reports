...
export default mixins(HexBinPlotBase, CanvasMixin).extend({
    ...
    methods: {
        getLabelFontSize(
            ...
            const maxLabel = (
                labels.map((l) => [l, cxt.measureText(l)]) as [
                    string,
                    TextMetrics
                ][]
            ).reduce((prev, current) =>
        ...
        draw() {
            ...
            this.$forceNextTick().then(() => {
                ...
                this.canvas.cxt.clearRect(
                    ...
                    0,
                    this.width,
                    this.scale.gl.height + this.margin.top
                );
                ...
                for (const cell of this.scale.gl) {
                    ...
                    const labelWidth = this.canvas.cxt.measureText(
                        cell.data
                    ).width;
                    ...
                    if (this.binned[cell.data] !== undefined) {
                        for (const h of this.binned[cell.data]) {
                            this.canvas.cxt.save();
                            this.canvas.cxt.translate(
                                h.x + cell.pos.paddingX / 2,
                                h.y + 20
                            );
