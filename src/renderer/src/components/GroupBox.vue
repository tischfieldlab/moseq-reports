                ...
                <b-list-group-item v-for="option in groups" :key="option.name">
                    <div :class="{ 'group-wrap': true, [option.style]: true }">
                        ...
                        >
                            <span
                                class="group-count"
                                :style="{ color: getContrast(option.color) }"
                                >{{ group_counts[option.name] }}</span
                            >
                        </div>
                        <b-popover
                            :target="$id(option.id)"
                        ...
                        >
                            <template v-slot:title
                                >Group Color ({{ option.name }})</template
                            >
                            <chrome-picker
                                :value="option.color"
                                @input="colorChangeHandler(option, $event)"
                                :disableAlpha="true"
                            />
                        </b-popover>
                        <span class="group_name" :title="option.name">{{
                            option.name
                        }}</span>
                    </div>
                </b-list-group-item>
        ...
        this.watchers.push(
            this.$store.watch(
                (state, getters) => {
                    ...
                    return {
                        c:
                            unnest(state, this.datasource).groupColors ||
                            ([] as string[]),
                        s:
                            unnest(state, this.datasource).selectedGroups ||
                            ([] as string[]),
                    };
                },
                (newValue) => {
                    if (newValue.s && newValue.c) {
                        this.groups.forEach((g) => {
                            ...
                            if (isSelected) {
                                g.color =
                                    newValue.c[newValue.s.indexOf(g.name)];
                            }
                        });
                    }
                },
    ...
    methods: {
        async buildGroups() {
            ...
            const availableGroups =
                this.$store.getters[`${this.datasource}/availableGroups`] || [];
            const selectedGroups =
                this.dataview !== undefined ? this.dataview.selectedGroups : [];
            const colorScale =
                this.dataview !== undefined ? this.dataview.groupColors : [];
            availableGroups.map((g, i) => {
                const sgi = new SelectableGroupItem(
                    g,
        ...
        updateGroups() {
            const groups = this.groups
                .filter((g) => g.selected)
                .map((g) => g.name);
            const colors = this.groups
                .filter((g) => g.selected)
                .map((g) => g.color);
            if (!deepEqual(groups, this.dataview.selectedGroups)) {
                this.$store.dispatch(
                    `${this.datasource}/updateSelectedGroups`,
                    {
                        groups,
                        colors,
                    }
