...
import LinePlotBase from "@render/components/Charts/LinePlot/LinePlotBase.vue";
  ...
  methods: {
    ...
    drawSeriesPoints(ctx: CanvasRenderingContext2D, sdata: object[]) {
      ...
      for (const node of sdata) {
        ctx.beginPath();
        ctx.arc(this.scale.x(node[this.varKey]), this.scale.y(node[this.valueKey]), this.pointSize, 0, 2 * Math.PI);
    ...
    drawAxisX(ctx: CanvasRenderingContext2D) {
      ...
      if (this.rotate_labels) {
        ...
        this.scale.x.domain().forEach((d) => {
          ctx.fillText(d, this.scale.x(d) + this.scale.x.bandwidth() / 2, 6);
        });
      }
