...
import { sankey, sankeyCenter, sankeyLeft, sankeyRight, sankeyJustify, SankeyLayout } from 'd3-sankey';
...
const graph = computed(() => {
  if (props.data.nodes.length > 0) {
    return sankeyGen.value(props.data);
  }
  return props.data;
});
...
const scale = computed(() => {
  ...
  if (props.linkColorMode === ColoringMode.Quantitative) {
    ...
    l = (link: Link) => color(li(link[props.linkColorProperty || 'value']) as string);
  } else {
    li = scaleOrdinal()
      .range(GetScale(props.categoricalColormap || 'schemeDark2') as string[])
      .domain(props.data.links.map((d) => d[props.linkColorProperty || 'id']));
    l = (link: Link) => color(li(link[props.linkColorProperty || 'id']) as string)!.brighter(0.5);
  }
  return { n, l, li };
});
...
const tooltip_text = computed(() => {
  if (hoverItem.value !== undefined) {
    return (props.tooltipFormatter || default_tooltip_formatter)(hoverItem.value, null);
  }
  return '';
});
...
function default_tooltip_formatter(hoverItem: Node | Link | undefined, that: any): string {
