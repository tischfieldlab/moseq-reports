<template>
  <g class="legend">
    ...
    <rect :x="-width / 2" :y="0" :width="width" :height="height" :fill="`url(${$idRef('color-gradiant')})`" />
    <g v-axis:[orientation]="linearscale" :transform="`translate(${axis_translate.x},${axis_translate.y})`" />
    <text class="label" :x="label_translate.x" :y="label_translate.y" :transform="`rotate(${label_translate.r})`">
      {{ title }}
    </text>
  </g>
</template>
...
import ColorScaleLegendBase, { Orientation } from "@render/components/Charts/Colors/ColorScaleLegendBase";
