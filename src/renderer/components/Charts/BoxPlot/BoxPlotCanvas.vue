<template>
  <div :style="{ width: '100%', height: '100%', overflow: 'hidden' }">
    ...
    <ToolTip :position="tooltipPosition" :show="hoverItem !== undefined">
      <div style="text-align: left" v-html="tooltip_text"></div>
    </ToolTip>
  </div>
</template>
...
export default mixins(BoxPlotBase, CanvasMixin).extend({
  ...
  methods: {
    ...
    drawBoxPlotNode(ctx: CanvasRenderingContext2D, node: GroupStats) {
      // Vertical midline
      ctx.beginPath();
      ctx.moveTo(this.scale.x(node.group) + this.halfBandwith, this.scale.y(this.fences.lower(node)));
      ctx.lineTo(this.scale.x(node.group) + this.halfBandwith, this.scale.y(this.fences.upper(node)));
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.moveTo(this.scale.x(node.group) + this.quaterBandwith, this.scale.y(this.fences.lower(node)));
      ctx.lineTo(this.scale.x(node.group) + this.quaterBandwith * 3, this.scale.y(this.fences.lower(node)));
      ctx.stroke();

      // Horizontal Median line
      ctx.beginPath();
      ctx.moveTo(this.scale.x(node.group), this.scale.y(node.q2));
      ctx.lineTo(this.scale.x(node.group) + this.halfBandwith * 2, this.scale.y(node.q2));
      ctx.stroke();

      // Horizontal Maximum line
      ctx.beginPath();
      ctx.moveTo(this.scale.x(node.group) + this.quaterBandwith, this.scale.y(this.fences.upper(node)));
      ctx.lineTo(this.scale.x(node.group) + this.quaterBandwith * 3, this.scale.y(this.fences.upper(node)));
      ctx.stroke();
    },
    drawPointNode(ctx: CanvasRenderingContext2D, node: DataPoint) {
      ctx.beginPath();
      ctx.arc(
        this.scale.x(node.group) + node.jitter + this.halfBandwith,
    ...
    drawAxisX(ctx: CanvasRenderingContext2D) {
      ...
      this.scale.x.domain().forEach((d) => {
        ...
        ctx.moveTo(this.scale.x(d) + this.halfBandwith, this.innerHeight);
        ctx.lineTo(this.scale.x(d) + this.halfBandwith, this.innerHeight + 6);
      });
      ...
      if (this.rotate_labels) {
        ...
        this.scale.x.domain().forEach((d, i) => {
          ctx.fillText(d, this.scale.x(d) + this.halfBandwith, this.innerHeight + 6);
        });
      }
