...
import { WhiskerType, GroupStats, DataPoint } from "@render/components/Charts/BoxPlot/BoxPlot.types";
...
export default mixins(LoadingMixin).extend({
  ...
  computed: {
    ...
    scale(): any {
      ...
      const x = scaleBand().domain(orderedLabels).range([0, this.innerWidth]).padding(0.2);

      const y = scaleLinear().domain(this.domainY).range([this.innerHeight, 0]);

      const w = scaleLinear().domain(this.domainKde).range([0, x.bandwidth()]);
  ...
  watch: {
    ...
    async point_size(newValue) {
      //   const result = await worker.swarm_points(
      //     this.points,
  ...
  methods: {
    prepareData(newData: DataPoint[]) {
      ...
      //   worker
      //     .prepareData(newData, this.innerHeight, this.point_size, this.groupLabels, this.show_points, this.kde_scale)
      //     .then((result) => {
      //       if (result !== undefined) {
      //         this.points = result.points;
      //         (this.groupedData = result.groupedData), (this.domainY = result.domainY);
      //         (this.domainKde = result.domainKde), this.compute_label_stats(this.groupLabels);
      //       }
      //     });
    },
    is_outlier(node: DataPoint): boolean {
      ...
      if (group) {
        return node.value < this.fences.lower(group) || node.value > this.fences.upper(group);
      }
      return false;
    },
