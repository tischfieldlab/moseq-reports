    ...
    <div>
        <svg v-show="hasData" :width="width" :height="height">
            <g :transform="`translate(${margin.left},${margin.top})`">
                <g v-for="cell in scale.gl" :key="cell.data" :data-group="cell.data" :transform="`translate(${cell.pos.x},${cell.pos.y})`" class="group">
                    <text :x="cell.pos.width / 2" :y="0" class="label">
                        {{ cell.data }}
                    </text>

                    <path v-for="h in binned[cell.data]"
                        :key="`${h.x}-${h.y}`"
                        :d="hexbing.hexagon()"
            ...
            <text :x="width / 2" :y="20" class="title">
                {{ title }}
            </text>
        </svg>
