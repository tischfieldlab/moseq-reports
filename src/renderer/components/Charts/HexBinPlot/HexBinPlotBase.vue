...
export default mixins(LoadingMixin).extend({
  ...
  computed: {
    ...
    scale(): any {
      const gl = gridLayout().size([this.innerSize.w, this.innerSize.h]).padding([0.3, 0.5]).aspect(1.0)(
        this.useGroups ? this.groupLabels : ["Overall"]
      );
      ...
      const c = scaleSequential(this.colormap).domain([0, this.zmax]);
