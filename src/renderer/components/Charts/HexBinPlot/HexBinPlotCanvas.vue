...
import ColorScaleLegend from "@rendercomponents/Charts/Colors/ColorScaleLegendCanvas.vue";
...
export default mixins(HexBinPlotBase, CanvasMixin).extend({
  ...
  methods: {
    getLabelFontSize(cxt: CanvasRenderingContext2D, maxWidth: number, labels: string[], fontFace: string) {
      const maxLabel = (labels.map((l) => [l, cxt.measureText(l)]) as [string, TextMetrics][]).reduce((prev, current) =>
        prev[1].width > current[1].width ? prev : current
      )[0];
      cxt.save();
    ...
    draw() {
      ...
      this.$forceNextTick().then(() => {
        ...
        // Draw each group
        let labelFontSize = this.getLabelFontSize(
          this.canvas.cxt,
