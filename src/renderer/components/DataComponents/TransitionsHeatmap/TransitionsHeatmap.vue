<template>
  <svg ref="canvas" :width="outsideWidth" :height="outsideHeight">
    ...
    <ColorScaleLegend
      ...
      :width="width / 2"
      :height="50"
      :transform="`translate(${outsideWidth / 2}, ${scale.o.bandwidth() + 40})`"
    />
  </svg>
</template>
...
export default mixins(LoadingMixin, WindowMixin).extend({
  ...
  computed: {
    ...
    scale(): any {
      ...
      const y = scaleBand()
        ...
        .range([o.bandwidth(), 0])
        .padding(0);
      const absz = scaleSequential(this.abs_colormap).domain([this.abs_vmin, this.abs_vmax]);
      const relz = scaleSequential(this.rel_colormap).domain([this.rel_vmin, this.rel_vmax]);
      const c = scaleLinear()
        .domain(absz.domain())
    ...
    dataspec() {
      ...
      if (this.dataview.countMethod === CountMethod.Usage) {
        ds = this.$store.getters[`datasets/resolve`]("transitions_usage");
      } else if (this.dataview.countMethod === CountMethod.Frames) {
        ds = this.$store.getters[`datasets/resolve`]("transitions_frames");
      } else {
  ...
  watch: {
    dataspec: {
      handler(newValue) {
        // LoadData(newValue[0], newValue[1], true).then((data) => this.prepare_data(data));
      },
      immediate: true,
    },
  },
