...
export default defineComponent({
  ...
  setup(props) {
    ...
    const items = computed(() => {
      const ids = store.getters[`${datasource.value}/selectedSyllableMap`];
      const clips = store.state.datasets.manifest?.syllable_clips?.manifest || [];
      return clips.filter((row: any) => row.sid_raw === ids.raw);
    });

    const args = computed(() => store.state.datasets.manifest?.syllable_clips?.args || {});
    const numExamples = computed(() => items.value.length);
    const currentItem = computed(() => items.value[exampleNum.value - 1]);

    const timeToSeconds = (time: string): number =>
      time.split(":").reduce((acc, t) => 60 * acc + parseFloat(t), 0);
    ...
    const sizeCalculated = (payload: { width: number; height: number }) => {
      ...
      store.commit(`${props.id}/updateAspectRatio`, { aspect_ratio: aspectRatio });
    };
