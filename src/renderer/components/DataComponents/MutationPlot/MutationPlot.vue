...
import { OrderingType, SortOrderDirection } from "@render/components/Charts/ClusteredHeatmap";
    ...
    syllable_ordering(): any[] {
      if (this.settings.syllable_order_type === OrderingType.Natural) {
        ...
        const minData = this.aggregateView.filter(
          (u) => u.group === this.settings.syllable_order_diff_minuend.toString()
        );
        const subData = this.aggregateView.filter(
          (u) => u.group === this.settings.syllable_order_diff_subtrahend.toString()
        );
