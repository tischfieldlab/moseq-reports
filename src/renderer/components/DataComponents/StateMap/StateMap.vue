  ...
  computed: {
    ...
    elements(): any[] {
      ...
      for (const [i, t] of trans.entries()) {
        if (this.activeSyllables.includes(t.row_id) && this.activeSyllables.includes(t.col_id)) {
          ...
          const relval = showRelDiff ? t.raw / transSum - relTrans[i].raw / relTransSum : t.raw / transSum;
    ...
    scale(): any {
      ...
      if (transExtent[0] < 0) {
        ...
        zo = scaleLinear().domain([0.0, 0.5, 1.0]).range([1.0, 0.0, 1.0]).clamp(true);

        o = scaleLinear().domain(transDomain).range([1.0, 0.0, 1.0]).clamp(true);

        z = scaleDiverging(GetScale(this.settings.colorscale) as (t: number) => string).domain(
          transDomain as [number, number, number]
        );
        zo = scaleDiverging(
        ...
        z = scaleSequential(GetScale(this.settings.colorscale) as (t: number) => string).domain(
          transDomain as [number, number]
        );
        zo = scaleSequential(GetScaleWithOpacity(this.settings.colorscale, o)).domain(transDomain as [number, number]);
      }
      return { r, t, o, z, zo };
    },
    ...
    sourceData(): any {
      ...
      const filterGroups = [...(showRelDiff ? [this.settings.plot_group, relDiffGroup] : [this.settings.plot_group])];
