...
import { BoxPlotSVG, BoxPlotCanvas, WhiskerType } from "@render/components/Charts/BoxPlot";
...
export default mixins(LoadingMixin, WindowMixin).extend({
  ...
  watch: {
    dataset: {
      handler(): any {
        // LoadData(this.dataset[0], this.dataset[1]).then((data) => (this.individualUseageData = data));
      },
      immediate: true,
    },
  },
