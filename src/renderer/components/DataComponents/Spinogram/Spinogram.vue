    ...
    <template v-if="has_data">
      ...
      <svg :width="outsideWidth" :height="outsideHeight">
        ...
        <g class="x-axis" v-axis:x="scale" :transform="`translate(${margin.left},${dims.y})`">
          <text class="label" :x="dims.w / 2" :y="35">Relative Lateral Position (mm)</text>
        </g>
        <g class="y-axis" v-axis:y="scale" :transform="`translate(${margin.left},${dims.y - dims.h})`">
          <text class="label" transform="rotate(-90)" :y="-45" :x="0 - dims.h / 2" dy="1em">Height (mm)</text>
        </g>
        <ColorScaleLegend
          title="Time (ms)"
          :scale="scale.t"
...
export default mixins(LoadingMixin, WindowMixin).extend({
  ...
  computed: {
    ...
    scale(): any {
      ...
      const t = scaleSequential(
        d3.interpolateRgb(rgb(c.r, c.g, c.b, ae[0]).toString(), rgb(c.r, c.g, c.b, ae[1]).toString())
      ).domain(te);
      return { x, y, t };
    },
    line(): any {
  ...
  watch: {
    dataspec: {
      handler() {
        // LoadData(this.dataspec[0], this.dataspec[1]).then((data: any[]) => {
