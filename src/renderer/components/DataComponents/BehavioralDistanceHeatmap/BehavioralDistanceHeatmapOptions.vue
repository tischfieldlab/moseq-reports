...
import { Colormap, ColumnOrdering, RowOrdering } from "@render/components/Charts/ClusteredHeatmap/Options";
  ...
  watch: {
    method_options_spec: {
      async handler(newValue) {
        // this.method_options = await LoadData(this.method_options_spec[0], this.method_options_spec[1]).then((cols) =>
        //   cols.filter((itm) => !itm.startsWith("row") && !itm.startsWith("col"))
        // );
      },
      immediate: true,
    },
