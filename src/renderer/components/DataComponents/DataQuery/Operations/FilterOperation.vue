  ...
  <div>
    <b-dropdown text="Add Filter" class="float-right add-filter-button" size="sm">
      <template v-for="col in columnOptions">
        <b-dropdown-item v-if="!columnAlreadyInFilter(col)" @click="addFilter(col)" :key="col">{{
          col
        }}</b-dropdown-item>
      </template>
    </b-dropdown>
...
<script lang="ts">
import { FilterOperation } from "@render/components/Core/DataLoader/DataLoader.types";
