<template>
    <div class="dataview-container">
        <BButton @click="toggleExpand" :title="isExpanded ? 'Collapse' : 'Expand'" variant="link"
            class="text-dark collapse-button text-decoration-none">
            <IBiChevronUp v-if="isExpanded" class="when-opened" title="Collapse" />
            <IBiChevronDown v-else class="when-closed" title="Expand" />
        </BButton>
        Results at this stage:
        <span class="datahint">{{ dataHint }}</span><br />
        <BCollapse v-model="isExpanded">
            <BFormTextarea v-model="formattedDataset" readonly rows="5" wrap="off" autocomplete="off" autocorrect="off"
