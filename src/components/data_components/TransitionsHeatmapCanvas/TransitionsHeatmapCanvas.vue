<template>
    <canvas ref="canvas" :width="outsideWidth" :height="outsideHeight">
...
RegisterDataComponent({
    friendly_name: 'Transitions Heatmap Canvas',
    component_type: 'TransitionsHeatmapCanvas',
...
export default mixins(LoadingMixin, WindowMixin).extend({
    ...
    computed: {
        ...
        scale(): any {
            ...
            const x = scaleBand()
                    ...
                    .rangeRound([0, o.bandwidth()])
                    .padding(0);
            const y = scaleBand()
                    .domain(range(100) as any[])
                    .rangeRound([o.bandwidth(), 0])
                    .padding(0);
    ...
    mounted() {
        ...
        if (ctx !== null) {
            this.startDrawing(ctx);
        }
    },
    methods: {
        group_transitions(group: string): number[][] {
        ...
        startDrawing(context: CanvasRenderingContext2D) {
            const draw = () => {
                if (Date.now() < (this.timestamp + 900)) {
                    return requestAnimationFrame(draw);
                }

                context.clearRect(0, 0, this.outsideWidth, window.outerHeight);

                for (const group of this.selectedGroups) {
                    const offset = this.scale.o(group);

                    const labelSize = context.measureText(group);
                    context.fillStyle = '#000000';
                    context.fillText(group,
                                     (offset + (this.scale.o.bandwidth() / 2) - (labelSize.width / 2)),
                                     10);

                    for (const [rid, rows] of this.group_transitions(group).entries()) {
                        for (const [cid, value] of rows.entries()) {
                            context.fillStyle = this.group_z(group)(value);
                            context.fillRect(this.scale.x(cid) + offset,
                                             this.scale.y(rid),
